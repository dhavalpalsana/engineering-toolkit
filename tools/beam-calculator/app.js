...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Link Sharing (versioned envelope when ToolExports is available)
  window.shareLink = () => {
    const state = captureCurrentState();
    const encoded = window.encodeToolShare
      ? window.encodeToolShare("beam-calculator", state)
  ...
  window.buildBeamMarkdown = () => {
    ...
    const lines = [
      ...
      `| Length L | ${s.L ?? L} m |`,
      `| E | ${s.E ?? E} GPa |`,
      `| Yield σy | ${s.yieldStrength ?? yieldStrength} MPa |`,
      `| Supports | ${(s.supports || supports || []).length} |`,
      `| Loads | ${(s.loads || loads || []).length} |`,
      ``,
      `_Generated by Engineering Toolkit · Beam Solver_`
    ];
    return lines.join("\n");
  };
  ...
  if (designParam) {
    try {
      ...
      if (window.decodeToolShare) {
        const res = window.decodeToolShare(designParam, "beam-calculator");
        if (!res.ok) {
        ...
        state = (window.decodeShareState ? window.decodeShareState(designParam) : JSON.parse(decodeURIComponent(escape(atob(designParam)))));
      }
      if (state) {
        loadStateObject(state);
        loadedFromUrl = true;
  ...
  // All export formats live in the header Export ▾ menu
  if (window.ToolExports) {
    window.ToolExports.register({
      ...
      report: () => window.exportAllDiagramsPdf && window.exportAllDiagramsPdf(),
      markdown: () => window.buildBeamMarkdown(),
      extra: [
        { id: "sfd-png", label: "SFD diagram (PNG)", run: () => window.exportDiagramPng("sfd-svg", "SFD") },
        { id: "bmd-png", label: "BMD diagram (PNG)", run: () => window.exportDiagramPng("bmd-svg", "BMD") },
        { id: "defl-png", label: "Deflection diagram (PNG)", run: () => window.exportDiagramPng("deflection-svg", "Deflection") }
      ],
